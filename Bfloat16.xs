...
SV * _fromBfloat16(pTHX_ __bf16 * in) {
  ...
  Newx(f_obj, 1, __bf16);
  if(f_obj == NULL) croak("Failed to allocate memory in _fromBfloat16 function");
  obj_ref = newSV(0);
...
SV * _oload_fmod(pTHX_ __bf16 * a, __bf16 * b, SV * third) {
...
void _deref2(pTHX_ mpfr_t * p, SV * base, SV * n_digits, SV * round) {
  ...
  if(!(SvIOK(base) && ((SvIVX(base) >= 2 && SvIVX(base) <= 62) || (SvIVX(base) >= -36 && SvIVX(base) <= -2))))
    croak("Second argument (base) supplied to Math::Bfloat16::_deref2 is not in acceptable range");
