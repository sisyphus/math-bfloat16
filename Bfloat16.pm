...
if(_MPFR_VERSION() < 262912 || !_buildopt_bfloat16_p()) {
  warn "Aborting: The underlying mpfr library (", _MPFR_VERSION_STRING(), ") does not support the __bf16 type";
  exit 0;
}
...
%Math::Bfloat16::handler = (
               1  => sub {return _fromBfloat16(shift)},
               2  => sub {return _fromIV(shift)},
               4  => sub {return _fromPV(shift)},
               3  => sub {return _fromNV(shift)},
               5  => sub {return _fromFloat32(shift)},
...
sub oload_fmod {
   my $itsa = _itsa($_[1]);
   return _oload_fmod(@_) if $itsa == 1;
   if($itsa < 6) {
     my $coderef = $Math::Bfloat16::handler{$itsa};
...
sub oload_interp {
   my $ret = bfloat16_get_str(bf16_to_MPFR($_[0]), 10, 0, 0); # MPFR_RNDN
   $ret =~ s/\@//g;
   return $ret;
}

sub bfloat16_get_str {
...
sub pack_bf16_hex {
  ...
  return Math::Bfloat16->new($is_neg . $prefix . substr($binstr,9, 7) . "p$power");
}
