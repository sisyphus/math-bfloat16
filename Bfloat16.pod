...
=head1 ASSIGNMENT FUNCTIONS
    ...
    $obj = Math::Bfloat16::new($val);
     ...
     If $val is not provided, then $obj takes the value NaN.
     $val can be an IV (integer), an NV (perl float), a PV (string),
     a Math::Bfloat16 object, a Math::Float32 object, a Math::MPFR
      object, a Math::GMPf object or a Math::GMPq object.
     See the SYNOPSIS section (above) for some elaboration,
     If necessary, $val will be rounded.
     Rounding will be towards nearest (with ties to even), unless
     $val is a Math::Float32 object - in which case it will be
     truncated (ie rounded towards zero).
     If you wish the Math::Float32 value to be assigned with
     rounding to nearest (ties to even) then assign it as:
       $obj = Math::Bfloat16::new(rndn_Float32($val));

    bf16_set($obj, $val);
     $val can be any one of the types that new() accepts.
     Set the value of $obj to the value specified by $val, rounded
     as detailed in the new() documentation.
...
=head1 ARITHMETIC
 ...
 The overloaded arithmetic operators accept IVs, NVs, PVs, Math::Float32
 objects and Math::Bfloat16 objects.
 Math::Float32 objects will be truncated (rounded towards zero) to
 Math::Bloat16 objects. If you want them to instead be rounded
 towards nearest with ties to even, then pass them as rndn_flt32($obj).

 The 'log', 'exp', 'sqrt', 'abs' and 'int' functions are also
 overloaded.
 Obviously, they accept only Math::Bfloat16 objects.
...
=head1 OUTPUT FUNCTIONS
   ...
   $obj = pack_bf16_hex($string);
    ...
    into a Math::Bfloat16 object whose value is the same as the
    object from which $string was derived.

   $nv = rndn_Float32($flt32); # $flt32 is a Math::Float32 object.
    Return the value of Math::Float32 object cast to an NV.
    Then, setting $bf16=Math::Bfloat16->new($nv) will assign the
    value of $flt32 rounded to nearest with ties to even.
    Setting $bf16=Math::Bfloat->new($flt32) will instead assign
    the truncated value - ie rounded towards zero.
...
=head1 MISCELLANEOUS FUNCTIONS
   ...
   $MPFR_OBJECT = bf16_to_MPFR($obj);
    Load Math-MPFR-4.43 or later before calling this function.
    Return the value of $obj as an 8-bit precision Math::MPFR object.
    This Math::MPFR object holds exactly the same value as $obj.

   $FLOAT32_OBJECT = bf16_to_Float32($obj);
    Load Math-Float32-0.02 or later before calling this function.
    Return the value of $obj as a 24-bit precision Math::Float32
    object.
    This Math::Float32 object holds exactly the same value as $obj.
