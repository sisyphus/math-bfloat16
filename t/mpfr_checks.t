...
BEGIN {
  ...
  Math::MPFR->import( @Math::MPFR::EXPORT_OK );
};
...
for my $v1(@p) {
  ...
  Rmpfr_set_BFLOAT16($mpfr1, $flt_1, 0);
  for my $v2(@p) {
    my $flt_2 = Math::Bfloat16->new($v2);
    Rmpfr_set_BFLOAT16($mpfr2, $flt_2, 0);
    SET_EMIN_EMAX();
    my $inex = Rmpfr_fmod($mpfr_rop, $mpfr1, $mpfr2, 0);
    Rmpfr_subnormalize($mpfr_rop, $inex, 0);
    RESET_EMIN_EMAX();
    Rmpfr_get_BFLOAT16($flt_rop, $mpfr_rop, 0);
...
# Test that subnormalize_generic
# fixes a known double-rounding anomaly.
my $s = '13.75e-41';
my $round = 0; # MPFR_RNDN
my $mpfr_anom1 = Rmpfr_init2(8);
Rmpfr_strtofr($mpfr_anom1, $s, 10, 0); # RNDN
my $anom1 = Math::Bfloat16->new($s);
cmp_ok(unpack_bf16_hex($anom1), 'eq', '0001', "direct assignment results in '0001'");
cmp_ok(unpack_bfloat16($mpfr_anom1, $round), 'eq', '0002', "indirect assignment results in '0002'");
cmp_ok($anom1, '!=', Math::Bfloat16->new($mpfr_anom1), "double-checked: values are different");
my $mpfr_anom2 = subnormalize_generic($s, @subn_args);
cmp_ok(unpack_bfloat16($mpfr_anom2, $round), 'eq', '0001', "subnormalize_generic() ok");
...
for my $c(@corners) {
  ...
  my $mpfr = subnormalize_generic($c, @subn_args);
