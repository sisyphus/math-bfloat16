...
cmp_ok($bf, '==', 42.5, "== NV");
cmp_ok($bf, '==', '42.5', "== PV");
cmp_ok($bf, '==', Math::Bfloat16->new(Math::MPFR->new(42.5)), "== from MPFR") if $have_mpfr;

cmp_ok($bf, '<', 44.5, "< NV");
cmp_ok($bf, '<', '44.5', "< PV");
cmp_ok($bf, '<', Math::Bfloat16->new(Math::MPFR->new(44.5)), "< from MPFR") if $have_mpfr;
...
cmp_ok($bf, '<=', Math::Bfloat16->new(Math::MPFR->new(42.5)), "<= from equiv MPFR") if $have_mpfr;
...
cmp_ok($bf, '>=', 40.5, ">= NV");
cmp_ok($bf, '>=', '40.5', ">= PV");
cmp_ok($bf, '>=', Math::Bfloat16->new(Math::MPFR->new(40.5)), ">= from MPFR") if $have_mpfr;
...
cmp_ok(($bf <=> Math::Bfloat16->new(Math::MPFR->new(42.5))), '==', 0, "<=> from equiv MPFR") if $have_mpfr;

cmp_ok(($bf <=> 40.5), '==', 1, "<=> smaller NV");
cmp_ok(($bf <=> '40.5'), '==', 1, "<=> smaller PV");
cmp_ok(($bf <=> Math::Bfloat16->new(Math::MPFR->new(40.5))), '==', 1, "<=> from smaller MPFR") if $have_mpfr;

cmp_ok(($bf <=> 44.5), '==', -1, "<=> bigger NV");
cmp_ok(($bf <=> '44.5'), '==', -1, "<=> bigger PV");
cmp_ok(($bf <=> Math::Bfloat16->new(Math::MPFR->new(44.5))), '==', -1, "<=> from bigger MPFR") if $have_mpfr;
